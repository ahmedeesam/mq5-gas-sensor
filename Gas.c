...
//CO.59,2.3*.36,4/LPG-.15,2.3*-.82,4(-.13,-.39)
float           LPGCurve[3]  = {2.3,-.15,-0.39};
	//float           LPGCurve[3]  = {2.3,0.2,-0.45};    //two points from LPG curve are taken point1:(200,1.6) point2(10000,0.26)
...
float           SmokeCurve[3] ={2.3,0.59,-0.13};
...
{                                                         // sensor and load resistor forms a voltage divider. so using analog value and load value
	return ( ((float)RL_VALUE*(1023-raw_adc)/raw_adc));     // we will find sensor resistor. [finding rs like user manual]
}
...
float SensorCalibration()
{
	...
	for (i=0;i<50;i++) {                   //take multiple samples and calculate the average value
		...
		val += ResistanceCalculation(adc_read());
		_delay_ms(500);
	}
	val = val/50;
...
float ReadSensor()
{
	...
	for (i=0;i<5;i++) {                                 // take multiple readings and average it.
		rs += ResistanceCalculation(adc_read());   // rs changes according to gas concentration.
		_delay_ms(50);
	}
