...
void lcd_init(void)
{
	...
	DATA_DDR = (1<<LCD_D4) | (1<<LCD_D3) | (1<<LCD_D2)| (1<<LCD_D1);
	CTL_DDR |= (1<<LCD_EN)|(1<<LCD_RW)|(1<<LCD_RS);

	
	DATA_BUS = (0<<LCD_D4)|(0<<LCD_D3)|(1<<LCD_D2)|(0<<LCD_D1);
...
{
	DATA_BUS = ((command & 0b11110000)>>3);
	CTL_BUS &=~(1<<LCD_RS);
	CTL_BUS |=(1<<LCD_EN);
	_delay_ms(1);
	CTL_BUS &=~(1<<LCD_EN);
	_delay_ms(1);
	DATA_BUS=((command & 0b00001111)<<1);
	CTL_BUS |=(1<<LCD_EN);
	_delay_ms(1);
	CTL_BUS &=~(1<<LCD_EN);
	_delay_ms(1);
...
void lcd_write_character(uint8_t character)
{
	...
	DATA_BUS = ((character & 0b11110000)>>3);
	CTL_BUS|=(1<<LCD_RS);
	CTL_BUS |=(1<<LCD_EN);
	_delay_ms(2);
	CTL_BUS &=~(1<<LCD_EN);
	_delay_ms(2);
	DATA_BUS=((character & 0b00001111)<<1);
	CTL_BUS |=(1<<LCD_EN);
	_delay_ms(2);
	CTL_BUS &=~(1<<LCD_EN);
	_delay_ms(2);
